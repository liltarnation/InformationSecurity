...
def round(a, b, c, x, mul, ts):
    fs = 0xffffffffffffffff
    c = c ^ x
    c = c & fs
    a -= ts[0][((c) >> (0*8))&0xFF] ^ ts[1][((c) >> (2*8))&0xFF] ^ ts[2][((c) >> (4*8))&0xFF] ^ ts[3][((c) >> (6*8))&0xFF]
    b += ts[3][((c) >> (1*8))&0xFF] ^ ts[2][((c) >> (3*8))&0xFF] ^ ts[1][((c) >> (5*8))&0xFF] ^ ts[0][((c) >> (7*8))&0xFF]
    b *= mul
    a = a & fs
    b = b & fs 
    c = c & fs
...
res = ""
for i in range(3):
    res += format(vals[i], '064b')
    
for i in range(0, 192, 8):
    result += chr(int(res[i:i+8], base=2))
